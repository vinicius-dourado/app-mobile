...
App mobile minimalista (Expo / React Native) que tira uma foto, envia diretamente para a API da OpenAI em um chat novo (sem histórico) e mostra a resposta na tela. Swipe lateral abre a câmera novamente.

## Fluxo

1. Tela inicial com ícone de câmera → toque para abrir a câmera.
2. Toque no botão branco do obturador → tira a foto.
3. A foto é enviada como uma nova requisição (`/v1/chat/completions`) para a OpenAI com o prompt:
...
Ao rodar no navegador, sua chave fica exposta no bundle e pode ser inspecionada. **Use só em desenvolvimento**. Para produção, sempre use backend proxy.

## Arquivos principais

- `App.js` — toda a UI e lógica (4 estados: home / camera / loading / result|error)
- `app.json` — configuração Expo + permissões de câmera
- `.env` — sua chave da OpenAI (não commitada)

## Modelo

Por padrão usa `gpt-4o-mini`. Para trocar, edite a constante `OPENAI_MODEL` em `App.js`.
